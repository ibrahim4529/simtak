...
@section('content')
    ...
    <section class="content">
        ...
        @endif
        @if($list_sidang_ta->isEmpty() && Auth::user()->isMahasiswa())
        ...
        @elseif(Auth::user()->isMahasiswa() && !Auth::user()->selesai_sidang_ta())
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box">
                    @if(Auth::user()->isMahasiswa() &&  (!Auth::user()->selesai_sidang_ta()))
                        <div class="box-body">
                            <div class="btn-group">
                                &nbsp;<a href="{{route('sidang-ta.create')}}" data-toggle="modal"
                                         class="config">
                    ...
                    <div class="box-body">
                        <table id="table_kp" class="table table-bordered table-striped">
                            ...
                            <tr align="center">
                                ...
                                <th>Judul Ta</th>
                                <th>Status</th>
                                @if(!Auth::user()->selesai_sidang_ta())
                                    <th>Aksi</th>
                                @endif
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($list_sidang_ta as $seminar_ta)
                                <tr>
                                    <td>1</td>
                                    <td>
                                    ...
                                    @if(!Auth::user()->selesai_sidang_ta())
                                        <td align="center">
                                            {!! Form::open(['method' => 'DELETE','route' => ['sidang-ta.destroy', $seminar_ta->id],'style'=>'display:inline']) !!}
                                            {!! Form::button('<i class="fa fa-trash fa-lg"></i>', ['class' => 'btn btn-sm btn-danger', 'type' =>'submit', 'title' =>'Hapus']) !!}
                                            {!! Form::close() !!}</i>
                                            @if(Auth::user()->isAdmin())
                                            {!! Form::open(['method' => 'PATCH','route' => ['sidang-ta.update', $seminar_ta->id],'style'=>'display:inline']) !!}
